...
def find\_or\_create\_holding\_bridge():
 ...
 return holding\_bridge When the inbound channel enters the application, we'll place it into our waiting bridge:

truepy79 wait\_bridge = find\_or\_create\_holding\_bridge()
 wait\_bridge.addChannel(channel=channel.id)When the dialed channel answers, we can remove the inbound channel from the waiting bridge - since there is only one waiting bridge being used, we can use `find_or_create_holding_bridge` to obtain it. We then place it into a newly created mixing bridge along with the dialed channel, in the same fashion as the `bridge-dial.py` example.
...
Hung up PJSIP/Bob-00000002JavaScript (Node.js)
--------------------
...
bridge-move.jstruejs /\*jshint node:true\*/
'use strict';
