...
Channels can have their audio recorded using the [`/channels/{channelId}/record`](https://wiki.asterisk.org/wiki/display/AST/Asterisk+13+Channels+REST+API#Asterisk13ChannelsRESTAPI-record) resource, and Bridges can have their audio recorded using the `[/bridges/{bridgeId}/record](https://wiki.asterisk.org/wiki/display/AST/Asterisk+13+Bridges+REST+API#Asterisk13BridgesRESTAPI-record)` resource.
...
vm-record.pypytrue#!/usr/bin/env python
...
client.run(apps=sys.argv[1])vm-record.jsjstrue/\*jshint node:true\*/
'use strict';
...
exten => \_3XX,1,NoOp()
 ...
 same => n,Hangup()This way, when calling any three-digit extension that begins with the number '3', the user will call into the application with the mailbox dialled (e.g. dialling "305" will allow the user to leave a message for mailbox "305").
...
recording\_state.pypytruefrom event import Event
...
class RecordingState(object):
 ...
 self.cleanup()
 self.recording.stop()
 self.call.state\_machine.change\_state(Event.DTMF\_OCTOTHORPE)recording\_state.jsjstruevar Event = require('./event')
...
ending\_state.pypytrueclass EndingState(object):
 state\_name = "ending"
 ...
 self.call.channel.hangup()ending\_state.jsjstruefunction EndingState(call) {
...
module.exports = EndingState;hangup\_state.pypytrueclass HungUpState(object):
 state\_name = "hungup"
 ...
 def enter(self):
 channel\_name = self.call.channel.json.get('name')
 print "Channel {0} hung up".format(channel\_name)hungup\_state.jsjstruefunction HungUpState(call) {
...
vm-call.pypytrue# At the top of the file
...
# Inside our VoiceMailCall class
 ...
 hungup\_state)
 self.state\_machine.start(recording\_state)vm-call.jsjstrue// At the top of the file
...
recording\_state.pypytrue def on\_dtmf(self, channel, event):
 digit = event.get('digit')
 if digit == '#':
 rec\_name = self.recording.json.get('name')
 print "Accepted recording {0} on DTMF #".format(rec\_name)
 ...
 self.call.state\_machine.change\_state(Event.DTMF\_STAR)recording\_state.jsjstrue function on\_dtmf(event, channel) {
 switch (event.digit) {
 case '#':
 console.log("Accepted recording", call.vm\_path);
 cleanup();
...
vm-call.pypytrue def setup\_state\_machine(self):
 hungup\_state = HungUpState(self)
 recording\_state = RecordingState(self)
 ending\_state = EndingState(self)
 self.state\_machine = StateMachine()
 ...
 recording\_state)
 self.state\_machine.start(recording\_state)vm-call.jsjstruethis.setup\_state\_machine = function() {
...
Cleaning up event handlers
Ending voice mail call from PJSIP/200-00000007silverseagreenReader Exercise 3solidblackWe have covered the `stop()` and `cancel()` methods, but live recordings provide other methods as well. In particular, there are `pause()`, which causes the live recording to temporarily stop recording audio, and `unpause()`, which causes the live recording to resume recording audio.
...
reviewing\_state.pypytrueimport uuid
...
class ReviewingState(object):
 ...
 self.call.state\_machine.change\_state(Event.DTMF\_STAR)reviewing\_state.jsjstruevar Event = require('./event');
...
Entering reviewing state
Accepted recording voicemail/305/1411501058.42 on DTMF #
Ending voice mail call from PJSIP/200-00000009silverseagreenReader Exercise 5solidblackIn the previous section we introduced the ability to delete a stored recording. Stored recordings have a second operation available to them: [copying](https://wiki.asterisk.org/wiki/display/AST/Asterisk+13+Recordings+REST+API#Asterisk13RecordingsRESTAPI-copyStored). The `copy()` method of a stored recording can be used to copy the stored recording from one location to another.
