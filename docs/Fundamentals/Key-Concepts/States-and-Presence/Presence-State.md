...
/// note
One thing to keep in mind with the `PRESENCE_STATE` dialplan function
is that, like with `DEVICE_STATE`, state may be queried from any
presence provider, but `PRESENCE_STATE` is only capable of setting
presence state for the `CustomPresence` presence state provider.
///
...
/// tip
Digium phones support only the available, away, dnd, xa, and chat
states. The unavailable and not_set states are not supported.
///
