...
| [suppress_moh_on_sendonly](#suppress_moh_on_sendonly)| Boolean| no| false| Suppress playing MOH to party A if party B sends "sendonly" or "inactive" in an SDP| |
...
| priv_key_file| Custom| | false| Private key file (TLS ONLY, not WSS)| |
...
A path to a .crt or .pem file can be provided. However, only the certificate is read from the file, not the private key. The 'priv\_key\_file' option must supply a matching key file.<br>
...
| user_agent| String| Asterisk PBX GIT-18-cf6d5fb618| false| Value used in User-Agent header for SIP requests and Server header for SIP responses.| |
