...
enable\_events=yes ; If enabled, recipients who joined the bridge via a channel driver
 ...
 ; This feature must also be enabled in user profiles.Of course, your configuration will be different but those are the parameters that need to be set.
...
You use the same mechanism as shown in [Conference Participant Messaging](https://wiki.asterisk.org/wiki/display/AST/Conference+Participant+Messaging).  The only difference will be that the message Content-Type will be "application/x-asterisk-confbridge-event+json".  The message bodies will be JSON events as shown above.
