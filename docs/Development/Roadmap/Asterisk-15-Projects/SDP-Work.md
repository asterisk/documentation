...
/\*
 ...
 \*/
int ast\_sdp\_state\_set\_connection\_address(struct ast\_sdp\_state \*state, struct ast\_sockaddr \*addr);Let's talk about the API a bit. The API introduces two new structures: `ast_sdp_state`, and `ast_sdp_options`.
...
/\*
 ...
 \* 
int ast\_rtp\_instance\_register\_ice\_new\_candidate\_cb(struct ast\_rtp\_instance \*rtp, new\_candidate\_fn cb, void \*data);This way, an RTP instance can be told by interested parties to be alerted whenever a new ICE candidate is learned. The data parameter is a way to quickly associate the RTP instance with another piece of data the callback cares about (like an SDP state or a PJSIP session).
