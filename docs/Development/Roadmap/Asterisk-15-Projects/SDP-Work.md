...
int init_session(struct my_channel_driver_session \*session)
{
 ...
 if (!sdp_state) {
 /\* ast_sdp_state_alloc() will free the options on failure \*/
 return -1;
 }
...
 
 ...
 /\* Let's pretend that my_channel_driver requires us to send a provisional response immediately upon receipt \*/
