...
POST /channels/12345/play?media=sound:tt-monkeys&media=sound:tt-weasels&media=number:555### Other Media Parameters
...
POST /playbacks/1283791327846/control?operation=next### Existing Operations
...
frame.htruecpp312  AST\_CONTROL\_STREAM\_STOP = 1000, /\*!< Indicate to a channel in playback to stop the stream \*/
 ...
 AST\_CONTROL\_STREAM\_NEXT = 1006, /\*!< Indicate to a channel in playback to play the next stream \*/* Update `channel.c` to consume and ignore the two new frame types
...
Update `file.c` `ast_waitstream_fr_cb` to accept an arbitrary `void *`. Update `waitstream_core` and its variants that pass a callback to also pass a `void *`. Modify `waitstream_control` to pass the `void *` to the callback.
