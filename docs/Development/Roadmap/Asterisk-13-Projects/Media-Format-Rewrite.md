...
 
struct ast\_codec \*ast\_codec\_get(const char \*name, enum ast\_format\_type type, unsigned int samples);Codec structures will be immutable once registered and created only once. If a user of the API wants to retrieve a codec they will use ast\_codec\_get with the provided information.
...
void ast\_format\_cap\_remove\_all(struct ast\_format\_cap \*cap);#### Capabilities structure iteration
