...
* : LeifMadsen Enterprises, Inc / Digium
...
* Group variables (Kobaz)
	...
	+ **Review 2011**: Code written and then re-written this year, tested in production for a year. Feels good code wise. Some suggestions on reviewboard and should be converted to ao2.
		...
		- <https://issues.asterisk.org/jira/browse/ASTERISK-15439>
* Pre-Dial (Kobaz) 
...
* SIP Security Events
	+ **Review 2011**: Additional work was updated and put into Asterisk 10. Only reported manager authentication events prior to Asterisk 10.
		...
		- Added to Asterisk 10. Reference: <https://issues.asterisk.org/jira/browse/ASTERISK-18264>
...
* Unique identifier for filtering log data to a call
	...
	+ **Review 2011**: Claude's patch was only for CLI filtering.
		- Discussion about in the logger.conf to change the configuration so that the 'core set verbose 5' (or debug, etc) that it does not affect all the configuration files when you just want to change the verbosity on the console. (<https://issues.asterisk.org/jira/browse/ASTERISK-18352>)
