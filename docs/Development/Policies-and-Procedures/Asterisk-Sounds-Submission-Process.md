...
8. Create an account on the project's bug tracker <https://github.com/asterisk/asterisk/issues/> if you do not already have one, and submit the file via that interface. In order to eliminate confusion, you should also [complete the license agreement on that system](/Asterisk+Issue+Guidelines#AsteriskIssueGuidelines-DigiumSubmissionLicenseAgreement), which is an electronic license agreement. You will obtain an issue number from the issues.asterisk.org system - save this number.
