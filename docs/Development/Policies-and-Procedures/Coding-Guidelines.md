...
Lines should be wrapped at no longer than 90 columns but shorter is fine if it improves clarity.
...
If you create new AMI events, please read [Asterisk Manager Interface (AMI)](/Configuration/Interfaces/Asterisk-Manager-Interface-AMI/). Do not re-use existing headers for new purposes, but please re-use existing headers for the same type of data.
...
Check [Asterisk Manager Interface (AMI)](/Configuration/Interfaces/Asterisk-Manager-Interface-AMI/) for more information on manager and existing headers. 
